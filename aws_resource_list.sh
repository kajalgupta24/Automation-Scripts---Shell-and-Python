...
# check if the required number of arugemts are passed
if[ $# -ne 2 ]; then
...
# Assign the service to variables and lowercase the service
aws_service = $(echo "$2" | tr '[:upper:]' '[:lower:]')
aws_region = $1

# Check if the AWS CLI is installed
if ! command -v aws &> /dev/null; then
   echo "AWS CLI is not installed. Please Install AWS CLI to run the script"
   exit 1
fi
